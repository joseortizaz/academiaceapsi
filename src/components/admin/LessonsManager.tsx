...
export function LessonsManager({ scope }: Props) {
  ...
  const { data: programas = [] } = useQuery({
    queryKey: ["lessons-manager", "programas", scope, user?.id],
  ...
  const { data: rows = [] } = useQuery<Leccion[]>({
    queryKey: ["lessons-manager", "lecciones", programaId],
    enabled: !!programaId,
    queryFn: async () => {
  ...
  const saveModulo = async (v: CourseModule) => {
    ...
    setOpenMod(false);
    qc.invalidateQueries({ queryKey: ["lessons-manager", "course_modules", programaId] });
  };
  ...
  const saveLeccion = async (v: Leccion) => {
    ...
    setOpenLec(false);
    qc.invalidateQueries({ queryKey: ["lessons-manager", "lecciones", programaId] });
  };
  ...
  return (
    <div>
      ...
      <div className="mb-6 max-w-md">
        ...
        {scope === "docente" && programas.length === 0 && (
          <p className="mt-2 text-sm text-muted-foreground">No tienes programas asignados aún.</p>
        )}
      </div>
        ...
        {(s, set) => (
          <>
            ...
            <div className="grid gap-2">
              ...
              />
              <p className="text-xs text-muted-foreground">
                Para diplomados con grupos escalonados: si el alumno pertenece a un grupo (cohorte) con fecha de inicio definida, esta lección se abrirá cuando pase esa cantidad de días desde el inicio del grupo. Si se define, prevalece sobre la fecha fija de arriba.
              </p>
            </div>
            <p className="text-xs text-muted-foreground">
