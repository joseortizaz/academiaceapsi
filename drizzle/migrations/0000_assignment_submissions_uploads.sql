create policy "assignment uploads: student insert own"
on storage.objects for insert to authenticated
with check (bucket_id = 'assignment-submissions' and (auth.uid())::text = (storage.foldername(name))[1]);

create policy "assignment uploads: student read own"
on storage.objects for select to authenticated
using (bucket_id = 'assignment-submissions' and (auth.uid())::text = (storage.foldername(name))[1]);

create policy "assignment uploads: student update own"
on storage.objects for update to authenticated
using (bucket_id = 'assignment-submissions' and (auth.uid())::text = (storage.foldername(name))[1])
with check (bucket_id = 'assignment-submissions' and (auth.uid())::text = (storage.foldername(name))[1]);

create policy "assignment uploads: staff read"
on storage.objects for select to authenticated
using (bucket_id = 'assignment-submissions' and (has_role(auth.uid(), 'admin'::app_role) or has_role(auth.uid(), 'docente'::app_role)));
...
BEGIN
  ...
  IF v_count = 0 THEN
    v_all_auto := false;
    SELECT coalesce(puntaje_maximo, 0) INTO v_total FROM assessments WHERE id = _assessment_id;
  END IF;
